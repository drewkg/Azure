...
Param(
    ...
    [string] $ResourceGroupName = 'ARM-DiagnosticSettings',
...
if ($ValidateOnly) {
  ...
  Write-Host @OptionalParameters
    New-AzSubscriptionDeployment -Name ((Get-ChildItem $TemplateFile).BaseName + '-' + ((Get-Date).ToUniversalTime()).ToString('MMdd-HHmm')) `
                                       -TemplateFile $TemplateFile `
                                       -TemplateParameterFile $TemplateParametersFile `
                                       @OptionalParameters `
                                       -Location ukSouth `
