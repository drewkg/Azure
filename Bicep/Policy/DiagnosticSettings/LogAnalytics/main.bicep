...
module DiagnosticsSQLELDeployPolicy './Modules/diagnostics-sqlel-deploy-policy.bicep' = {
...
module DiagnosticsSQLMIDBDeployPolicy './Modules/diagnostics-sqlmidb-deploy-policy.bicep' = {
  name: 'diagnostics-sqlmidb-deploy-policy'
}

module DiagnosticsSRCHDBDeployPolicy './Modules/diagnostics-srch-deploy-policy.bicep' = {
  name: 'diagnostics-srch-deploy-policy'
}

module DiagnosticsSTDeployPolicy './Modules/diagnostics-st-deploy-policy.bicep' = {
...
module DiagnosticsTRAFDeployPolicy './Modules/diagnostics-traf-deploy-policy.bicep' = {
  name: 'diagnostics-traf-deploy-policy'
}

module DiagnosticsTSIDeployPolicy './Modules/diagnostics-tsi-deploy-policy.bicep' = {
  name: 'diagnostics-tsi-deploy-policy'
}

module DiagnosticsVGWDeployPolicy './Modules/diagnostics-vgw-deploy-policy.bicep' = {
  name: 'diagnostics-vgw-deploy-policy'
}

module DiagnosticsVMDeployPolicy './Modules/diagnostics-vm-deploy-policy.bicep' = {
  name: 'diagnostics-vm-deploy-policy'
}

module DiagnosticsVMSSDeployPolicy './Modules/diagnostics-vmss-deploy-policy.bicep' = {
...
module DiagnosticsLogAnalyticsPolicySet './Modules/diagnostics-loganalytics-deploy-initiative.bicep' = {
  ...
  params: {
    policyDefinitionId: [
      ...
      extensionResourceId(tenantResourceId('Microsoft.Management/managementGroups', split(reference('getManagementGroupName', '2020-10-01', 'Full').scope, '/')[2]), 'Microsoft.Authorization/policyDefinitions', DiagnosticsWVDWORKSPACEDeployPolicy.outputs.name)
    ]
  }
}
